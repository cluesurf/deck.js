...
import fs from 'fs'
import * as gfs from '~/code/tools/files'
import crypto from 'node:crypto'
import { type Cafs } from '~/code/store/types'
  ...
  try {
    switch (message.type) {
      ...
      case 'readPkgFromCafs': {
        ...
        try {
          pkgFilesIndex =
            loadJsonFile<PackageFilesIndex>(filesIndexFile)
        } catch {
    ...
    if (calculatedHash !== normalizedRemoteHash) {
      return {
        ...
        error: {
          ...
          algorithm: algo,
          expected: integrity,
          found: `${algo}-${Buffer.from(calculatedHash, 'hex').toString(
            'base64',
          )}`,
        },
      }
...
function addFilesFromDir({
  ...
  readManifest,
  files,
}: AddDirToStoreMessage): AddFilesFromDirResult {
  if (!cafsCache.has(cafsDir)) {
    cafsCache.set(cafsDir, createCafs(cafsDir))
  }
  const cafs = cafsCache.get(cafsDir)!
...
type ProcessFilesIndexResult = {
  filesIntegrity: Record<string, PackageFileInfo>
  filesMap: Record<string, string>
}

function processFilesIndex(
  filesIndex: FilesIndex,
): ProcessFilesIndexResult {
